...
user_input = ''
# file_path = 'assets/images/cvs/'+filename
with open('samples/sample_cv_1.pdf','rb') as pdf_file:
...
results = isc.main('data scientist', 'remote')
...
final_df = pd.DataFrame(columns = [['JobID', 'JobTitle', 'Company', 'PostDate',
                                    'ExtractDate', 'Summary', 'JobURL', 'Likelihood']])
# final_df = pd.DataFrame(columns=[[results.columns, 'Likelihood']])
print(final_df.columns)
# print(pd.DataFrame(columns=results.columns.append('Likelihood')).columns)
# job_probs_df = pd.DataFrame({'Likelihood': top_jobs.values})
...
for top_job_id, job_value in top_jobs.items():
    ...
    for index, job in results.iterrows():
        if (top_job_id == job['JobID']):
            job.append({'Likelihood': job_value})
            print(job)
